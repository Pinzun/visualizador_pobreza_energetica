...
def calcular_indicador(filtro, session):
	resultado = session.query(
		AsequibilidadData.GASTO_10P
	).filter(filtro).scalar()

	# Checkeo de None para evitar errores en cálculos
	gasto_10 = resultado or 0

	return {
		"indicador": formato_chileno_prom(gasto_10)
	}


def obtener_valores_tabla(filtro, session):
	# Para esta tabla, se desplegará el gasto 10%
...
def obtener_indicador_gasto_10p(filtro, session):
