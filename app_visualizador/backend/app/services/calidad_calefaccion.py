...
def calcular_indicadores_censo(filtro, session):
    total_sinacceso = session.query(
        ...
        func.sum(CalidadCenso.CAL_GAS + CalidadCenso.CAL_PARAFINA +
                 CalidadCenso.CAL_LENIA + CalidadCenso.CAL_CARBON +
                 CalidadCenso.CAL_NO_DECLARA)
    #El filtro que se aplica acá es el que se recibe como parámetro para definir si
    # es un cálculo nacional, regional o comunal.
...
def calidad_tipo_combustible_censo(filtro, session):
    ...
    electricidad = _sum(CalidadCenso.CAL_ELECTR)
    solar = _sum(CalidadCenso.CAL_SOLAR)
    gas = _sum(CalidadCenso.CAL_GAS)
    parafina = _sum(CalidadCenso.CAL_PARAFINA)
    lenia = _sum(CalidadCenso.CAL_LENIA)
    pellet = _sum(CalidadCenso.CAL_PELLET)
    carbon = _sum(CalidadCenso.CAL_CARBON)
    no_tiene = _sum(CalidadCenso.CAL_NO_TIENE)
    no_declara = _sum(CalidadCenso.CAL_NO_DECLARA)
