# services/asequibilidad_med_nac_proporcion.py
...
def calcular_indicador(filtro, session):
	resultado = session.query(
		AsequibilidadData.MED_NAC_PROPORCION
	).filter(filtro).scalar()

	# Checkeo de None para evitar errores en cálculos
	med_nac_proporcion = resultado or 0

	return {
		"med_nac_proporcion": formato_chileno_prom(med_nac_proporcion)
	}


def obtener_valores_tabla(filtro, session):
	# Para esta tabla, se desplegará el valor de la media nacional proporcion
	...
	for macrozona in range(1, 5):  # Macrozonas 1-4
		...
		if resultado:
			med_nac_proporcion, _cut_reg = resultado
...
def obtener_indicador_med_nac_proporcion(filtro, session):
